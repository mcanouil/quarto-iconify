...
- fix: Render nothing in Typst output when the icon set name or the icon name is invalid. The name is a path segment in the Iconify request and in the cache file name, so a name with `/` or `..` wrote the cached file outside the cache directory. (#100)
